...
if ($richtig == "true" && $id > 0)
{
    ...
    $par[0] = $this->user->getId();
    $par[1] = $id;
    $res = $this->sql->insert("INSERT INTO fortschritt (benutzer_id, karte_id, gewusst) VALUES(?, ?, 1)", "ii", $par);
}
elseif($richtig == "false" && $id > 0)
{
    ...
    $par[0] = $this->user->getId();
    $par[1] = $id;
    $res = $this->sql->insert("INSERT INTO fortschritt (benutzer_id, karte_id, gewusst) VALUES(?, ?, 0)", "ii", $par);
}
